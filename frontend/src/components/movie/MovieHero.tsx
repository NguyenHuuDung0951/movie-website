...
export const MovieHero = ({ movie, mediaType = "movie" }: Props) => {
  ...
  return (
    <section className="relative overflow-hidden rounded-2xl bg-zinc-950">
      ...
      <div className="relative z-10 grid min-h-100 grid-cols-1 px-4 pb-6 pt-8 sm:px-6 md:min-h-125 md:grid-cols-[1fr_260px] md:px-8 md:pt-10">
        <div className="flex flex-col justify-between gap-6">
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3">
            ...
            <Link
              to={watchPath}
              className="inline-flex items-center gap-2 rounded-full border border-white/25 bg-white/10 px-4 py-2.5 text-sm font-semibold text-zinc-100 backdrop-blur transition hover:bg-white/20"
            >
        ...
        <div className="mt-6 hidden justify-end md:flex">
          <img
            ...
            alt={title}
            className="h-85 w-57.5 rounded-xl border border-zinc-700/80 object-cover shadow-2xl shadow-black/70"
            loading="lazy"
          />
        </div>
      </div>
