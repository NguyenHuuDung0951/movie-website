...
const MovieCard = ({ movie }: { movie: MovieItem }) => {
  return (
    <article className="group rounded-2xl border border-zinc-200
     bg-white p-4 shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
      <div className="mb-3 h-36 rounded-xl bg-gradient-to-br
  ...
  return (
    <main className="relative overflow-hidden
     bg-[radial-gradient(circle_at_top_left,#ffedd5_0%,#fff7ed_25%,#ffffff_60%)]">
      <section className="mx-auto grid w-full max-w-6xl gap-8 px-4 pb-14 pt-12
       sm:px-6 lg:grid-cols-[1.2fr_0.8fr] lg:items-center">
        <div>
          ...
          <p className="mt-4 max-w-xl text-base text-zinc-700 sm:text-lg">
            Tìm phim hot, lọc theo thể loại, lưu danh sách yêu thích và theo dõi các review mới
            nhất trên một giao diện nhanh, gọn và dễ dùng.
          </p>
          <div className="mt-7 flex flex-wrap gap-3">
            <Link href="/login" className="rounded-full bg-zinc-900
         ...
         border-zinc-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 space-y-3">
            {hotMovies.slice(0, 3).map((movie, index) => (
              <div key={movie._id} className="flex items-center justify-between rounded-xl
               bg-zinc-50 px-4 py-3">
                ...
                <span className="rounded-full bg-zinc-900 px-2 py-1 text-xs font-semibold text-white">HOT</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="hot" className="mx-auto w-full max-w-6xl px-4 pb-12 sm:px-6">
        <div className="mb-5 flex items-center justify-between gap-3">
          ...
          <Link href="/" className="text-sm font-semibold text-zinc-700 underline-offset-2 hover:underline">
            Xem tất cả
          </Link>
        </div>
